...
module.exports = async function(req, res) {
  ...
  try {
    // የ API ኪዩን (Key) ከ Vercel Settings ላይ ይወስዳል
    const apiKey = process.env.GEMINI_API_KEY || process.env.VITE_GEMINI_API_KEY;
    
    if (!apiKey) {
      return res.status(500).json({ error: 'የ API Key አልተገኘም! እባክዎ Vercel Settings ላይ ያስገቡ።' });
    }
    ...
    // የ Google Gemini API ሊንክ (Gemini 1.5 Flash model)
    ...
    // የተመለሰውን መልስ ለዌብሳይታችን (Frontend) መላክ
