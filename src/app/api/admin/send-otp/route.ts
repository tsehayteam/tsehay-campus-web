...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("⚡ ADMIN OTP FOR eyoubsahle@gmail.com:", otpCode);
    ...
    // 5. Build Branded HTML Email Template (Tsehay Brand: #f9b03c Golden Orange, #3268ba Cobalt Blue)
      ...
      try {
        const res = await fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({
            from: fromEmail,
            to: [cleanEmail],
            subject: `🔐 Tsehay Campus Admin OTP Code: ${otpCode}`,
            html: emailHtml
          })
        });
        ...
        console.warn('Resend admin OTP mail attempt notice:', mailErr);
      }
    }
    ...
    return NextResponse.json({
      success: false,
      error: error.message || 'OTP code dispatch failed'
    }, { status: 500 });
  }
}
