...
export default function ComingSoonCoursesSection({
  ...
  return (
    <section id={id} className={`py-20 sm:py-28 bg-transparent border-b border-white/[0.08] relative overflow-hidden ${className}`}>
      ...
      <div className="max-w-[1340px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-7">
          {displayedCourses.map((course, index) => {
            const courseVideo = course.video || course.videoUrl || course.previewVideoUrl;
            ...
            return (
              ...
              >
                ...
                >
                    ...
                    >
                      ...
                      {courseVideo && (
                        <button
                          ...
                          className="absolute inset-0 z-25 flex items-center justify-center bg-black/35 hover:bg-black/50 transition cursor-pointer group/playbtn"
                          title="የቪዲዮ ቅድመ-ዕይታ ይመልከቱ (Watch Teaser Video)"
                        >
                          <div className="w-12 h-12 sm:w-13 sm:h-13 rounded-full bg-gradient-to-tr from-[#f9b03c] via-amber-400 to-yellow-300 text-slate-950 flex items-center justify-center pl-1 text-base shadow-[0_0_30px_rgba(249,176,60,0.8)] group-hover/playbtn:scale-115 transition-all border border-amber-100">
                  ...
                  {/* Primary CTA: ተጠባባቂ ዝርዝር ውስጥ ግባ (Join Waitlist) + Optional Teaser Video Button */}
                  <div 
                    className="p-5 sm:p-6 pt-0 mt-auto space-y-2"
                  ...
                  >
                    {courseVideo && (
                      <button
                        ...
                        className="w-full py-2.5 px-3 rounded-xl bg-slate-900/90 hover:bg-slate-800 text-amber-300 hover:text-amber-200 font-bold text-xs border border-amber-400/30 hover:border-amber-400/60 transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer shadow-sm group/vbtn"
                      >
                        <i className="fa-solid fa-circle-play text-red-500 text-sm group-hover/vbtn:scale-115 transition-transform"></i>
                        <span>ቪዲዮ ቅድመ-ዕይታ (Watch Teaser)</span>
                      </button>
                    )}
                    <button
                      type="button"
      ...
      {activeVideoCourse && (
        <CinematicVideoModal
          isOpen={Boolean(activeVideoCourse)}
          onClose={() => setActiveVideoCourse(null)}
          videoUrl={activeVideoCourse.video || activeVideoCourse.videoUrl || activeVideoCourse.previewVideoUrl}
          title={activeVideoCourse.title}
          poster={activeVideoCourse.image}
        />
      )}
    </section>
  );
