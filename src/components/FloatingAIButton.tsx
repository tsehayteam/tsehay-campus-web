...
export default function FloatingAIButton() {
  ...
  const handleFaqScroll = () => {
    ...
    setCanScrollFaqLeft(scrollLeft > 10);
    setCanScrollFaqRight(scrollWidth - scrollLeft - clientWidth > 10);
  };

  const scrollFaq = (direction: 'left' | 'right') => {
    if (!faqScrollRef.current) return;
    const offset = direction === 'left' ? -220 : 220;
    faqScrollRef.current.scrollBy({ left: offset, behavior: 'smooth' });
  };
  ...
  // 🎙️ 5. Seamless Real-Time Voice Input (Direct transcription into input box, Zero popups)
  ...
  // 🔊 6. Natural, Warm Ethiopian / Pleasant Female Voice Reader for Tsehay AI
  const playAiVoiceResponse = (text: string, idx: number) => {
    ...
    // Clean text: strip markdown code blocks, links, bullets, asterisks, emojis
  ...
  const tryAudioEndpoint = (cleanText: string, idx: number) => {
    try {
      ...
      const audioUrl = `/api/ai/tts?text=${encodedText}&lang=${aiLang === 'en' ? 'en' : 'am'}`;
  ...
  // 🚫 Completely remove Tsehay AI floating launcher icon from the Admin Dashboard, Classroom/Dashboard player, and Maintenance page
  if (pathname?.startsWith('/admin') || pathname?.startsWith('/dashboard') || pathname?.startsWith('/classroom') || pathname?.startsWith('/maintenance')) {
    return null;
  }
        ...
        <div className="mb-4 w-[94vw] sm:w-[460px] md:w-[500px] bg-[#070b14]/95 backdrop-blur-3xl border border-white/20 rounded-3xl shadow-[0_25px_80px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden transition-all duration-300 relative h-[640px] sm:h-[700px] max-h-[90vh] animate-in fade-in slide-in-from-bottom-4 duration-200">
              ...
              return (
                ...
                >
                  <div className={`flex items-end gap-2 w-full ${isUser ? 'justify-end' : 'justify-start'}`}>
                    ...
                    <div 
                      className={`p-3.5 sm:p-4 rounded-2xl text-xs sm:text-[13px] leading-relaxed shadow-sm break-words overflow-visible max-w-[95%] sm:max-w-[92%] ${
                        isUser
                    ...
                    >
                      ...
                      {/* Rich Formatted AI / User Message - Auto Expand Full View */}
                      {isUser ? (
                        <div className="whitespace-pre-wrap font-body break-words">
                          {m.text}
                        </div>
                      ) : (
          ...
          <div className="relative px-3 py-2 bg-gradient-to-r from-[#070c18] via-[#0d162a] to-[#070c18] border-t border-white/10 z-10">
            ...
            <div 
              ...
              className="flex items-center gap-2 overflow-x-auto tsehay-ai-scrollbar pb-1 pt-0.5"
            ...
            >
              {quickPrompts.map((qp, i) => (
                <button
                  ...
                  className="text-[11px] font-bold text-gray-200 hover:text-white bg-[#0f172a] hover:bg-[#f9b03c]/20 hover:border-[#f9b03c]/70 px-3 py-1.5 rounded-xl border border-white/15 whitespace-nowrap transition-all duration-200 active:scale-95 cursor-pointer shrink-0 shadow-sm flex items-center gap-1.5 hover:shadow-[0_0_15px_rgba(249,176,60,0.3)]"
                >
                  <span className="w-1.5 h-1.5 rounded-full bg-[#f9b03c]"></span>
                  <span>{qp.label}</span>
                </button>
              ))}
            </div>
          ...
          >
            ...
            <div className="relative flex-1">
              <input
                ...
                className={`w-full bg-white/5 border rounded-2xl px-3.5 py-2.5 text-xs sm:text-sm text-white placeholder-gray-400 outline-none transition duration-300 ${
                  isRecordingVoice 
                    ? 'border-[#f9b03c] ring-2 ring-[#f9b03c]/40 shadow-[0_0_20px_rgba(249,176,60,0.35)] animate-pulse placeholder-amber-300' 
                    : 'border-white/15 focus:border-[#f9b03c] focus:ring-1 focus:ring-[#f9b03c]/30'
                }`}
              />
            </div>
            ...
            <button
              ...
              className={`w-10 h-10 rounded-2xl border flex items-center justify-center transition-all duration-300 active:scale-90 cursor-pointer shrink-0 ${
                isRecordingVoice
              ...
              title={isRecordingVoice ? "ድምፅ ማዳመጥ አቁም (Stop Listening)" : "በድምፅ ተናገር (Speak via Voice)"}
            >
              <i className={`fa-solid ${isRecordingVoice ? 'fa-microphone-lines text-base' : 'fa-microphone text-sm'}`}></i>
            </button>
            ...
            {(input.trim() || attachedImage) && (
              <button
                type="submit"
                disabled={isLoading}
                className="h-10 px-4 rounded-2xl font-black text-xs flex items-center justify-center gap-1.5 transition-all duration-200 cursor-pointer shadow-lg active:scale-90 shrink-0 bg-gradient-to-r from-[#f9b03c] to-amber-400 text-slate-950 shadow-[0_0_20px_rgba(249,176,60,0.5)] hover:brightness-110 animate-in zoom-in-95"
                title="መልዕክት ላክ"
              >
                {isLoading ? (
