  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 py-2 sm:py-2.5 flex flex-col md:flex-row items-center justify-between gap-2.5 md:gap-4 text-xs sm:text-sm">
        ...
        {/* Banner Text with Animated Sparkle */}
        <div className="flex items-center gap-2 text-center md:text-left min-w-0 font-primary">
          <span className="shrink-0 flex items-center justify-center w-6 h-6 rounded-full bg-amber-500/15 border border-[#f9b03c]/30 text-base select-none">
            🌼
          </span>
          <div className="leading-snug">
            <span className="text-gray-100">
              <strong className="text-[#f9b03c] font-black tracking-wide">እንኳን ለአዲሱ ዓመት አደረሳችሁ!</strong>{' '}
              ለSHEIN Importing Business ኮርስ <strong className="text-emerald-400 font-extrabold">15% ቅናሽ</strong> ለማግኘት ፕሮሞ ኮድ ይጠቀሙ{' '}
              <span className="inline-block text-[11px] sm:text-xs text-amber-200/80 font-medium px-1.5 py-0.5 rounded-full bg-white/5 border border-white/10 ml-0.5">
                (ለ10 ተማሪዎች ብቻ!)
              </span>
            </span>
          </div>
        ...
        <div className="flex items-center gap-2 sm:gap-2.5 shrink-0 flex-wrap justify-center">
          ...
          {/* Interactive Code Pill with 1-Click Copy */}
          <button
            type="button"
            onClick={handleCopy}
            className={`group relative flex items-center gap-1.5 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-lg border text-xs font-mono font-bold tracking-wider transition-all duration-200 cursor-pointer select-none active:scale-95 ${
              copied
                ? 'bg-emerald-500/20 border-emerald-400 text-emerald-300 shadow-[0_0_12px_rgba(52,211,153,0.3)]'
                : 'bg-white/10 hover:bg-white/15 border-[#f9b03c]/50 hover:border-[#f9b03c] text-white shadow-sm'
            }`}
            title="ኮዱን ኮፒ ለማድረግ ይጫኑ"
          >
            <Sparkles className="w-3.5 h-3.5 text-[#f9b03c] shrink-0 animate-pulse hidden sm:inline-block" />
            <span className="text-[#f9b03c] font-black">{promoCode}</span>
            ...
            <span className={`text-[11px] font-semibold transition-colors ${copied ? 'text-emerald-300' : 'text-white/80'}`}>
              {copied ? 'Copied!' : 'Copy'}
            </span>
          </button>
